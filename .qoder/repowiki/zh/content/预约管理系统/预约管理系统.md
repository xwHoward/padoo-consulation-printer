...
- 轮牌管理云函数（manageRotation）：初始化、取号、服务完成、位置调整。
...
participant CF as "云函数<br/>getHistoryData/matchCustomer"
participant DB as "云数据库"
User->>Page : 触发操作如加载历史/匹配客户
Page->>Utils : 调用工具方法
...
- 云函数之间存在协作：getHistoryData 依赖 consultation_records、customer_membership、membership_usage；getAvailableTechnicians 依赖 reservations、consultation_records、schedule、rotation_queue。
- 类型定义贯穿前后端，保证数据结构一致性。
