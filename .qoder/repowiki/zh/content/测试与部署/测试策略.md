...
participant F as "云函数<br/>login/getAll/manageRotation"
...
- 集成测试
  - 覆盖率：云函数与数据库交互建议≥70%，重点覆盖 getAll、轮牌、登录、统计。
  - 数据隔离：使用测试环境数据库与独立集合，测试后清理。
- 端到端测试
  - 覆盖率：页面主流程≥90%，包括登录、表单、保存、打印、轮牌、推送。
  - 场景：弱网、断网、蓝牙不可用、超时重试。
- 性能测试
...
## 结论
通过分层测试策略与完善的 Mock/数据管理，可在保证质量的同时提升开发效率。建议以单元测试为基础、集成测试为中坚、端到端测试为保障，配合性能与压力测试，构建可持续的质量闭环。
...
- 回归：每日运行全量回归测试，重点关注登录、数据读写、轮牌与打印。