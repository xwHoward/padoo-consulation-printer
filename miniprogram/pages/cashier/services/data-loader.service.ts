...
export class CashierDataLoaderService {
	...
	async loadInitialData(): Promise<void> {
		await loadingService.withLoading(this.page, async () => {
			...
			// 并行获取所有基础数据
			const [allRooms, , , allStaff, technicianRes] = await Promise.all([
				app.getRooms(),
	...
	async prepareRotationList(today: string): Promise<RotationItem[]> {
		...
		const nowStr = formatTime(new Date(), false);
	...
	private calculateAvailableTime(
		...
		const occupiedSlots = [
			...consultations.map(c => ({ start: c.startTime, end: c.endTime })),
			...reservations.map(r => ({ start: r.startTime, end: r.endTime }))
		].sort((a, b) => a.start.localeCompare(b.start));

		// 找到从当前时间开始，或今日班次开始时间之后的第一个可用时间段
		const startTime = laterOrEqualTo(nowStr, shiftStart) ? nowStr : shiftStart;

		const availableSlots: string[] = [];

		for (let i = 0; i <= occupiedSlots.length; i++) {
			const slotStart = i === 0 ? startTime : occupiedSlots[i - 1].end;
			const slotEnd = i === occupiedSlots.length ? shiftEnd : occupiedSlots[i].start;

			if (slotEnd >= shiftEnd) {
				break;
			}

			const actualStart = earlierThan(slotStart, startTime) ? startTime : slotStart;
			const actualEnd = laterOrEqualTo(slotEnd, shiftEnd) ? shiftEnd : slotEnd;
