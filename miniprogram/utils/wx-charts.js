...
function getPieDataPoints(series) {
    ...
    var isZeroTotal = !(count > 0);
    series.forEach(function (item) {
        item.data = item.data === null ? 0 : item.data;
        item._proportion_ = isZeroTotal ? 0 : item.data / count * process;
    });
    series.forEach(function (item) {
...
function getXAxisPoints(categories, opts, config) {
    ...
    // guard: avoid division by zero when categories is empty
...
function getDataPoints(data, minRange, maxRange, xAxisPoints, eachSpacing, opts, config) {
    ...
    data.forEach(function (item, index) {
        if (item === null) {
            ...
            var rangeSpan = (maxRange - minRange);
            if (!rangeSpan) { rangeSpan = 1; }
            var height = validHeight * (item - minRange) / rangeSpan;
            height *= process;
...
function drawCanvas(opts, context) {
    // guard: avoid white screen/freeze on some devices when draw throws
    try {
        context.draw();
    } catch (e) {
        try {
            setTimeout(function () {
                try { context.draw(); } catch (err) { }
            }, 16);
        } catch (err) { }
    }
}
...
var Charts = function Charts(opts) {
    ...
    // guard defaults for width/height
    if (typeof opts.width !== 'number' || opts.width <= 0 || typeof opts.height !== 'number' || opts.height <= 0) {
        try {
            var _sys = wx.getWindowInfo();
            if (typeof opts.width !== 'number' || opts.width <= 0) {
                opts.width = _sys.windowWidth;
            }
            if (typeof opts.height !== 'number' || opts.height <= 0) {
                opts.height = Math.max(200, Math.floor(_sys.windowWidth * 0.6));
            }
        } catch (e) {
            opts.width = opts.width || 320;
...
Charts.prototype.showToolTip = function (e) {
    ...
    if (this.opts.type === 'line' || this.opts.type === 'area') {
        // throttle tooltip redraws
        var now = Date.now();
        this._lastTooltipTime = this._lastTooltipTime || 0;
        if (now - this._lastTooltipTime < 50) {
            return;
        }
        this._lastTooltipTime = now;
...
Charts.prototype.scroll = function (e) {
    // TODO throtting...
    if (e.touches[ 0 ] && this.opts.enableScroll === true) {
        // throttle to avoid excessive redraw
        var now = Date.now();
        this._lastScrollTime = this._lastScrollTime || 0;
        if (now - this._lastScrollTime < 30) {
            return;
        }
